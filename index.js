...
function getdata() {
    ...
    fetch(
    "http://localhost:3000/posts?_start=" + initialPost + "&_end=" + endPost)
    .then((res) => {
...
function deleteContent( element) {
    ...
    console.log("uno");
    fetch("http://localhost:3000/posts?id=1", requestOptions)
...
function open(json, element, modalheader, userModal, emailModal) {
  // deleteContent (json,element)
    ...
    btnShowComents.addEventListener("click", () =>
        showComents(element, url1, btnShowComents)
    );
}
