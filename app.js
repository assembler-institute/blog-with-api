...
async function getPosts() {
    ...
    postInfo.forEach((element) => {
        container.innerHTML += `<div class="card"><p class="title" id="${element.id}" data-bs-toggle="modal" data-bs-target="#staticBackdrop">${element.title}</p><div><button class="btn btn-warning edit" data-bs-toggle="modal" data-bs-target="#editing" data-edit="${element.id}">Edit</button><button class="btn btn-danger delete" data-bs-toggle="modal" data-bs-target="#exampleModal" data-delete="${element.id}">Delete</button></div></div>`;
    });
    modalDrawer();
    deleteButton();
    editButton();
