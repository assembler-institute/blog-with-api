...
async function getPosts() {
    ...
    postInfo.forEach((element) => {
        container.innerHTML += `<div class="card m-2 p-2 text-center backg"><p class="title m-1 fs-5" id="${element.id}" data-bs-toggle="modal" data-bs-target="#staticBackdrop">${element.title}</p><div><button class="btn btn-warning btn-sm edit m-1" data-bs-toggle="modal" data-bs-target="#editing" data-edit="${element.id}">&#10000;</button><button class="btn btn-sm btn-danger delete m-1" data-bs-toggle="modal" data-bs-target="#exampleModal" data-delete="${element.id}">&#128465;</button></div></div>`;
    });
    modalDrawer();
    deleteButton();
    editButton();
                        ...
                        if (target.id == postInfo[j].id) {
                            ...
                            if (postInfo[j].userId == usersInfo[h].id) {
                                ...
                                if (postInfo[j].id == commentsInfo[i].postId) {
                                    ...
                                    comment.innerHTML = `
                  <p><strong>${commentsInfo[i].name}</strong><br>
                  ${commentsInfo[i].body}<br>
                  <i>${commentsInfo[i].email}</i></p>`;
    ...
    confirmDelete.addEventListener("click", async() => {
        ...
        try {
            const resp = await axios.delete(
                `http://localhost:3000/posts/${iddDelete}`
            );
